...
Angular RealtimeService
   ↓ SignalR connection + JWT access token
RealtimeHub
   ↓
IUserIdProvider maps connection to user id
   ↓
IRealtimeNotifier
   ↓
SignalRRealtimeNotifier
   ↓
Clients.User / Clients.Group / Clients.All
   ↓
Angular latestMessage signal
   ↓
Feature UI update without refresh
```
...
```text
Progress: ~99.8% (Project 9.8 Generic Realtime Notification with SignalR Complete)
```
...
❌ Realtime Notification Center / unread notifications
❌ Extend NSwag usage ไปยัง feature services อื่น
...
Project 9.9 Extend NSwag usage ไปยัง feature services อื่น
Optional: Audit Report PDF Export
```
...
JWT + Refresh Token + Role + Permission DB Mapping + Security Admin APIs/UI + Better Error UX + Auto Retry + Layer Separation + Redis Cache + Advanced Filtering + Hangfire Jobs + Audit Trail + Kendo Grid + Custom Grid Adapter + Route UX + Reporting + Excel Export + PDF Export + Async Report Generation + NSwag Client Generation
```
...
Next: Project 9.8.1 Realtime Notification Center
```
