...
## ✅ Project 14: Audit SaveChanges Interceptor

### สิ่งที่ทำ

```text
สร้าง AuditSaveChangesInterceptor (EF Core SaveChangesInterceptor)
   - Added: auto-stamp CompanyID จาก tenant (ถ้าว่าง) + CreatedBy/ModifiedBy/Date จาก current user
   - Modified: set ModifiedBy/ModifiedDate + กัน CreatedBy/CreatedDate ถูกทับ (IsModified=false)
ลงทะเบียน interceptor (scoped) ใน AddDbContext ผ่าน (sp, options) + AddInterceptors
ลบโค้ด audit + CompanyID ที่เขียนมือใน EmployeeService.Create/Update (DRY + security)
Tests: stamp tenant+audit on create / ไม่ override CompanyID ที่ระบุเอง
```

### Skill ที่ได้

```text
EF Core SaveChangesInterceptor (SavingChanges / SavingChangesAsync)
ChangeTracker.Entries<T>() + EntityState (Added/Modified)
Write-side tenant stamping (สมมาตรกับ read-side query filter)
Scoped interceptor via AddDbContext (sp, options) overload
PropertyEntry.IsModified กัน field ถูกเขียนทับ
```

### follow-up

```text
✅ Backend: ลบ CompanyID/CreatedBy (CreateEmployeeRequest) + ModifiedBy (UpdateEmployeeRequest)
   + validators + tests อัปเดต (27 tests เขียว)
⏳ Frontend: ต้อง regen NSwag (เปิด API ก่อน) แล้วลบ field ใน employee-create/edit + models/employee.ts
   (ตอนนี้ frontend ยังทำงาน — backend ignore field ส่วนเกิน)
```
...
## ✅ Project 17: Optimistic Concurrency — Integration Test (SQL จริง)

```text
สถานะ: ฟีเจอร์ optimistic concurrency มีอยู่แล้วครบ loop (backend+frontend) แต่ไม่มี test
  - BaseEntity.RowVersion [Timestamp]; UpdateEmployeeRequest/EmployeeResponse.RowVersion
  - EmployeeService.UpdateAsync: Entry(e).Property(x=>x.RowVersion).OriginalValue = request.RowVersion
    (กัน reload-then-save trap) → catch DbUpdateConcurrencyException → ConcurrencyConflictException
  - EmployeesController catch → 409 ConflictError; frontend employee-edit เก็บ+ส่ง rowVersion กลับ
ปัญหา: EF InMemory บังคับ concurrency token ไม่ได้ → unit test ธรรมดาพิสูจน์ไม่ได้ ต้องใช้ SQL จริง
ทำ: integration test ต่อ SQL Server จริง (azure-sql-edge ใน docker, port 1433)
  - SqlServerFixture : IAsyncLifetime — สร้าง DB ชื่อ random ต่อ run (EnsureCreatedAsync) + drop ตอน DisposeAsync
  - IClassFixture<T> แชร์ fixture ทั้ง class; CreateContext() เปิด AppDbContext ใหม่ต่อ "request"
  - EmployeeConcurrencyTests: seed → 2 user ได้ staleRowVersion เดียวกัน → A save สำเร็จ →
    B save ด้วย version เก่า → Assert.ThrowsAsync<ConcurrencyConflictException> (กัน lost update)
ไฟล์: tests/MXHRM.Infrastructure.Tests/Common/SqlServerFixture.cs (ใหม่),
  tests/MXHRM.Infrastructure.Tests/Employees/EmployeeConcurrencyTests.cs (ใหม่)
ผล: 31 tests เขียว (App 6 + Infra 19 + Api 6)
ทักษะใหม่: integration testing กับ DB จริง, Testcontainers-style fixture, IAsyncLifetime,
  IClassFixture, throwaway database per run, ทำไม InMemory ทดสอบ concurrency ไม่ได้
```

### follow-up

```text
DeleteAsync ยังไม่เช็ค RowVersion → ถ้าจะกัน "ลบ row ที่คนอื่นเพิ่งแก้" ต้องเพิ่ม If-Match/RowVersion ให้ Delete
integration test ยังต้องมี SQL ทำงาน (make infra-up) — ยังไม่ self-contained ใน CI
